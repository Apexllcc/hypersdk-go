...
func TestAccountQueriesUseOfficialWiresAndDecodeDecimalResponses(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if request.User != "0xabc" && request.Type != "vaultDetails" {
			t.Fatalf("unexpected user %q", request.User)
		}
		switch request.Type {
		case "portfolio":
		...
		case "userFunding":
			if request.StartTime == nil || *request.StartTime != 1 || request.EndTime == nil || *request.EndTime != 2 {
				t.Fatalf("unexpected userFunding request: %#v", request)
			}
		...
		case "vaultDetails":
			if request.Vault != "0xvault" {
				t.Fatalf("unexpected vault %q", request.Vault)
			}
		...
		default:
			t.Fatalf("unexpected request type %q", request.Type)
		}
	}))
	defer server.Close()
