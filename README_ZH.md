...
context 取消和连接关闭，且不会阻塞入站确认、事件或 pong。订阅写入使用有界
deadline；关闭流程会先关闭活动 socket，再等待写入 goroutine 结束。
...
如有多个客户端共用同一 IP，可通过 `transport.NewWeightLimiter(capacity, window)`
创建共享预算，再传给 `WithRateLimitPolicyAndLimiter`。
